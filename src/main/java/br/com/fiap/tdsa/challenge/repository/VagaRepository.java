...
public class VagaRepository {
	...
	public List<VagaDTO> listarVagas() {
		...
		try {
			...
			if (rs.isBeforeFirst()) {
				while (rs.next()) {
					list.add(new VagaDTO(rs.getString("NM_VAGA"), rs.getString("DS_DESCRICAO"),
							rs.getDate("DT_CRIACAO"), rs.getBoolean("ST_PRESENCIAL"), rs.getString("ST_QUALIFICACAO"),
							rs.getString("DS_SALARIO"), rs.getString("DS_TIPO_VAGA")));
				}
			}
